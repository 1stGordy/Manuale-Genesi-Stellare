...
# Configurazione Completa Scheda Personaggio 

## 1. Header (Parte Alta - Fissa)

Questa zona gestisce i dati vitali e le meccaniche "sempre attive".

### Dati Identificativi & Trigger

|**ID Campo**|**Nome Visualizzato**|**Tipo Input**|**Opzioni / Fonte**|**Logica Backend (Trigger)**|
|---|---|---|---|---|
|`char_name`|**Nome**|Testo|-|-|
|`char_img`|**Avatar**|Upload|-|Visualizzazione tonda.|
|`char_gender`|**Genere**|Dropdown|M, F, T, TT|-|
|`char_class`|**Classe**|Dropdown|_Vedi DB Classi_|Imposta `hit_die`. Attiva risorse specifiche (es. _Dadi Prodezza_ se Guerriero-Commando).|
|`char_spec`|**Specializzazione**|Dropdown|_Dipendente da Classe_|Filtra poteri e privilegi.|
|`char_race`|**Razza**|Dropdown|_Vedi DB Razze_|Applica bonus caratteristiche. Imposta `char_size`.|
|`char_archetype`|**Archetipo**|Dropdown|_Dipendente da Razza_|Applica bonus secondari.|
|`char_level`|**Livello**|Numero|1-20|Calcola `prof_bonus`. Sblocca slot/punti. Link a `xp_bar`.|
|`char_align`|**Allineamento**|Dropdown|LB, NB, CB, LN, N, CN, LM, NM, CM|-|
|`char_size`|**Taglia**|Dropdown|S, M, L, XL|Default da Razza. Modificabile.|
|`char_age`|**Età**|Numero|-|-|

### Vitalità & Progressione

|**ID Campo**|**Nome**|**Visual**|**Logica Funzionale**|
|---|---|---|---|
|`xp_bar`|**EXP**|Barra Gialla|`xp_current` / `xp_next_req`.<br><br>  <br><br>_Logica:_ Se `current` > `req`, Livello +1 e overflow su nuovo livello.|
|`hp_bar`|**Punti Ferita**|Barra Rossa|**Max:** (`hit_die` * `lvl`) + (`mod_con` * `lvl`).<br><br>  <br><br>**Current:** Input danni/cure. Cambia colore se <50% (Giallo) o 0 (Grigio).|
|`hp_temp`|**PF Temp**|Overlay Viola|Si consumano prima dei PF reali. Reset a 0 su Riposo Lungo.|
|`hit_dice`|**Dadi Vita**|Contatore|**Max:** Uguale al Livello. **Tipo:** Da Classe (d6/d8/d10).<br><br>  <br><br>_Uso:_ Spendi per recuperare PF durante Riposo Breve.|
|`death_saves`|**TS Morte**|3 Checkbox|3 Successi / 3 Fallimenti. Reset se HP > 0.|
|`exhaustion`|**Indebolimento**|Contatore (0-6)|Livello di fatica. Applica penalità ai tiri (da regole base).|

### Statistiche & Derivati

|**ID**|**Nome**|**Valore Base**|**Modificatore**|**Note Logiche**|
|---|---|---|---|---|
|`str`|**Forza**|1-20+|`(Val-10)/2`|-|
|`dex`|**Destrezza**|1-20+|`(Val-10)/2`|Influenza CA e Iniziativa.|
|`con`|**Costituzione**|1-20+|`(Val-10)/2`|Influenza PF e Dadi Vita.|
|`int`|**Intelligenza**|1-20+|`(Val-10)/2`|Casting (Melder, Specialista).|
|`wis`|**Saggezza**|1-20+|`(Val-10)/2`|Casting (Ingegnere, Mistico).|
|`cha`|**Carisma**|1-20+|`(Val-10)/2`|Casting (Paradosso, Solariano).|

|**ID**|**Nome**|**Formula**|**Note**|
|---|---|---|---|
|`init`|**Iniziativa**|`mod_dex` + `bonus_vari`|-|
|`ac`|**Classe Armatura**|`base_armor` + `shield` + `mod_dex` (cap) + `misc`|Gestire Cap Destrezza per armature Medie (+2) e Pesanti (+0).|
|`prof`|**Competenza**|Tabella Livelli (+2 a lv1, +6 a lv20)|Si somma se competente.|
|`dc_save`|**CD Esper**|8 + `prof` + `mod_casting`|_Nascosto se la classe non ha poteri._|

### Tiri Salvezza (Box Rapido)

_Visualizzare 6 righe piccole accanto alle stat._

- **Check:** Competenza (Sì/No - Auto da Classe).
    
- **Valore:** `mod_stat` + (`prof` se competente) + `misc`.
    

---

## 2. Navbar (Menu Navigazione)

Bottoni per cambiare la vista inferiore:

[Privilegi] [Abilità] [Talenti] [Esper] [Armi] [Equip] [Veicoli] [Note]

---

## 3. Parte Inferiore (Contenuto Dinamico)

### Tab: Privilegi (Info & Competenze)

- **Lista Privilegi:** Generata automaticamente da Classe, Razza e Livello.
    
- **Lingue:** Input testuale (Default da Razza).
    
- **Competenze Armi:** [ ] Semplici [ ] Marziali [ ] Altro.
    
- **Competenze Armature:** [ ] Leggere [ ] Medie [ ] Pesanti [ ] Scudi.
    

### Tab: Abilità

Tabella con colonne: _Competenza (Check)_, _Nome_, _Caratteristica_, _Bonus Totale_.

- **Logica:** Se Check=True, Bonus = `mod` + `prof`. Se "Attitudine Naturale" (Specialista), Bonus = `mod` + (`prof` * 2).
    

### Tab: Talenti (Feats)

- **Lista:** Card con Nome e Descrizione.
    
- **Aggiungi:** Popup filtrato per prerequisiti (es. "Richiede Forza 13").
    

### Tab: Esper (Poteri & Risorse)

_Questa sezione deve cambiare drasticamente in base alla Classe._

**A. Configurazioni Generiche**

- **Slot Tecniche (Ingegnere, Specialista-Artificio):**
    
    - Griglia Slot: Livello 1-9. Input: `Max` (Auto) / `Usati` (Manuale).
        
- **Punti Talento (Melder, Paradosso, Mistico):**
    
    - Contatore Unico: `Max` / `Attuali`.
        
- **Lista Poteri:** Card (Nome, Costo/Livello, Tempo, Gittata, Durata, Descrizione).
    

B. Risorse di Classe Specifiche (Tracker Personalizzati)

Da mostrare in alto nella tab Esper o Privilegi se la classe corrisponde.

|**Classe/Spec**|**Nome Risorsa**|**Tipo**|**Logica**|
|---|---|---|---|
|**Ingegnere**|_Espansione del Rig_|Checkbox (1-3)|Reset: Riposo Breve.|
|**Mistico**|_Fonte Vitale_|Pool Dadi (d6)|Max = Livello. Reset: Riposo Lungo.|
|**Guerriero**|_Azione Impetuosa_|Checkbox (1-2)|Reset: Riposo Breve.|
|**Guerriero**|_Recuperare Energie_|Checkbox (1)|Reset: Riposo Breve.|
|**Commando**|_Dadi Prodezza_|Pool Dadi (d8/d10)|Max = Variabile. Reset: Riposo Breve.|
|**Storm Guard**|_Impulso Arma_|Checkbox (2)|Reset: Riposo Breve.|
|**Solariano**|_Sintonizzazione_|Toggle|[Fotone] / [Gravitone] / [Neutro].|
|**Solariano**|_Round in Sint._|Contatore (1-3+)|Necessario per attivare "Poteri Zenit" al round 3.|
|**Paradosso**|_Campo Distorsione_|Checkbox (2)|Reset: Riposo Breve.|
|**Paradosso**|_Ancora Realtà_|Toggle|[Attiva/Disattiva] (Conferisce Vantaggio TS).|

### Tab: Armi

- **Statistiche Globali:**
    
    - Mischia: `mod_str` + `prof`.
        
    - Distanza: `mod_dex` + `prof`.
        
- **Tabella:** Nome, Gittata, Danni (es. 2d6+3), Tipo, Munizioni (Max/Residue).
...
- **Inventario:** Tabella (Nome, Quantità, Peso, Note).
    
- **Ingombro (Opzionale):** Somma pesi vs (Forza * 15).
...
- **Tabella:**
    ...
    - Nome, Taglia, Velocità, Carico.
        
    - **CA:** Base + Manovrabilità + `mod_dex` (Pilota).
        
    - **HP:** Dadi Veicolo + (2 * `int` Tecnico).
        
    - **IS (Integrità):** Base + `mod_wis` (Tecnico).
        

---

# Appendice Database (Logica)

## Classi & Risorse

_Per Antigravity: associare questi valori al dropdown "Classe"._

1. **Ingegnere (Sag):** Tecniche (Slot), 1d8 DV. Risorse: Espansione Rig.
    
2. **Melder (Int):** Canalizzazione (Punti), 1d6 DV.
    
3. **Specialista (Int):** Nessuna (Base) / Tecniche (Artificio), 1d8 DV.
    
4. **Guerriero (For/Cos):** Manovre (Solo Commando), 1d10 DV. Risorse: Azione Impetuosa, Recuperare Energie.
    
5. **Mistico (Sag):** Canalizzazione (Punti), 1d8 DV. Risorse: Fonte Vitale (d6 pool).
    
6. **Solariano (Car):** Rivelazioni (Speciali), 1d10 DV. Risorse: Sintonizzazione (Stato).
    
7. **Paradosso (Car):** Canalizzazione (Punti), 1d6 DV. Risorse: Campo Distorsione.
    

## Razze & Modificatori

_Per Antigravity: associare questi valori al dropdown "Razza"._

1. **Ashenforged:** Cos +2. Taglia M.
    
2. **Eldori:** Car +2. Taglia M. (Archetipi: Lunare Int+1, Solare Sag+2).
    
3. **Umani:** Variabile (Vedi archetipi). Taglia M.
    
4. **Kesh:** Des +2. Taglia M.
    
5. **Promethean:** Des +2. Taglia M.
    
6. **Androidi:** Int +2. Taglia M.
    
7. **Talandri:** Sag +2. Taglia M.
    
8. **Pahtra:** Des +2. Taglia M.
    
9. **Vesk:** For +2. Taglia M.
    
10. **Ysoki:** Des +2. Taglia S.
    

## XP Table (Rollover)

Logica: Total_XP è il valore assoluto. Level è calcolato su Total_XP.

Barra_Display = Total_XP - XP_Soglia_Livello_Corrente.

(Esempio: A 1500 XP totali (Liv 3), la barra mostra 600/1800, perché il lv 3 parte da 900 e finisce a 2700).

# Appendice A: Caratteristiche e Exp
Le caratteristiche sono 6: 

| Caratteristica         | Cosa Rappresenta                                          |
| :--------------------- | :-------------------------------------------------------- |
| **FORZA (For)**        | Potenza fisica, atletica, danni in mischia.               |
| **DESTREZZA (Dex)**    | Agilità, riflessi, sparare con armi a distanza, pilotare. |
| **COSTITUZIONE (Cos)** | Resistenza fisica, salute, sopravvivenza al vuoto.        |
| **INTELLIGENZA (Int)** | Memoria, logica, hacking, conoscenze tecnologiche.        |
| **SAGGEZZA (Sag)**     | Intuizione, percezione, forza di volontà, medicina.       |
| **CARISMA (Car)**      | Personalità, inganno, comando, capacità sociale.          |
Ogni caratteristica ha un punteggio grezzo che è collegato secondo la tabella che segue ad un modificatore che viene poi usato per qualsiasi prova
Tabella: Punteggi e Modificatori
...
|   18-19   |      +4      |
|    20     |      +5      |
Quando troviamo scritto Aumento punteggio caratteristica, quello che aumenta è il punteggio grezzo, non il modificatore

Quella sotto stante è la tabella dei punti esperienza necessari per salire di livello
...
| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Animare Oggetti (Sciame)** | 1 azione | 36 metri | Concentrazione, fino a 1 minuto | Lanci in aria fino a 10 piccoli oggetti (o droni inattivi) che prendono vita sotto il tuo controllo. Possono attaccare i nemici (Attacco +8, Danni 1d8+4) o proteggerti. |
| **Colpo Chirurgico** | 1 azione | Arma | Istantanea | Effettui un attacco con un'arma. Se colpisci, il bersaglio deve superare un TS Costituzione (CD alta) o subire **7d8 danni extra** e essere stordito per 1 round. Se il bersaglio ha meno di 50 PF, il danno è raddoppiato. |
| **Interfaccia Mentale (Bond)** | 1 azione | 9 metri | 1 ora | Colleghi i cervelli di fino a 8 creature consenzienti tramite un network neurale criptato. Potete comunicare telepaticamente a qualsiasi distanza (stesso piano). |
| **Modifica Memoria (Neuralyzer)** | 1 azione | 9 metri | Concentrazione, fino a 1 minuto | Un flash di luce e onde ipnotiche permettono di riscrivere fino a 10 minuti della memoria recente di un bersaglio che fallisce un TS Saggezza. |
| **Ologramma Avanzato** | 1 azione | Personale | Concentrazione, fino a 1 ora | Diventi invisibile mentre un doppio olografico perfetto appare al tuo posto. L'ologramma si muove, parla e reagisce (controllato mentalmente). |

#### Grado 6
| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Campo Anti-Tecnologia** | 1 azione | Personale (Sfera 3m) | Concentrazione, fino a 10 minuti | Generi un campo EMP costante. All'interno della sfera, sensori, armi a energia, costrutti e comunicazioni cessano di funzionare o hanno svantaggio (se creature). |
| **Disintegrazione Focalizzata** | 1 azione | 18 metri | Istantanea | Un raggio di antimateria colpisce un bersaglio. TS Destrezza. Fallimento: **10d6 + 40 danni da forza**. Se ridotto a 0 PF, il bersaglio diventa polvere. |
| **Visore a Raggi X** | 1 azione bonus | Personale | 1 ora | Vedi attraverso illusioni, oscurità magica, porte segrete e puoi vedere creature invisibili o nascoste entro 36 metri. |

#### Grado 7
| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Evasione Temporale** | 1 reazione | Personale | Istantanea | _(Quando vieni colpito)_ Attivi un dispositivo sperimentale che riavvolge la tua posizione di pochi secondi. Il danno o l'effetto viene annullato e ti teletrasporti in uno spazio libero entro 9 metri. |
| **Gabbia di Forza** | 1 azione | 30 metri | 1 ora | Crei una prigione cubica (3m) di forza invisibile e indistruttibile intorno a un nemico. Nessun TS per evitarla se è abbastanza grande da contenerlo. Blocca ogni uscita fisica e magica. |

#### Grado 8
| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Bomba a Singolarità** | 1 azione | 45 metri | Istantanea | Lanci un dispositivo che collassa lo spazio. Sfera di 6 metri. TS Destrezza o subire **10d6 danni da forza e 10d6 danni contundenti**, ed essere risucchiati al centro. |
| **Clone (Backup Digitale)** | 1 ora | Tocco | Istantanea | Crei un corpo clone inerte in una vasca criogenica e carichi la tua coscienza su un server sicuro. Se muori, la tua mente viene scaricata nel clone entro 1d4 ore. |

#### Grado 9
| Nome                    | Tempo di attivazione | Gittata   | Durata     | Descrizione                                                                                                                                                                                                        |
| :---------------------- | :------------------- | :-------- | :--------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Arresto del Tempo**   | 1 azione             | Personale | Istantanea | Attivi un acceleratore cronale estremo. Ti muovi così velocemente che il mondo sembra fermo. Hai **1d4 + 1 turni consecutivi** in cui puoi muoverti e agire liberamente (se attacchi qualcuno, l'effetto finisce). |
| **Esecuzione Perfetta** | 1 azione             | 18 metri  | Istantanea | Analizzi ogni micro-movimento del bersaglio e prevedi il futuro immediato. Se il bersaglio ha meno di 100 PF, muore istantaneamente al tuo attacco. Se ha più PF, subisce **10d10 danni**.                         |


### Tecniche Specifiche per Specializzazione (Specialista)

_Disponibili solo per gli specialisti che hanno scelto il rispettivo archetipo._

#### Archetipo: Infiltrato
| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Camminare nei Muri** (Grado 5) | 1 azione | Personale | 1 ora | I tuoi atomi vibrano in fase. Puoi attraversare legno, pietra e metallo (non piombo) come se fossero aria. |
| **Ombra Vivente** (Grado 6) | 1 azione | Personale | 8 ore | Finché sei in luce fioca o oscurità, sei invisibile ai sensori elettronici e alla vista normale. Puoi usare la tua azione bonus per teletrasportarti tra le ombre fino a 36 metri. |
| **Occultamento Mentale** (Grado 8) | Passiva | Personale | Permanente | Sei immune a qualsiasi forma di divinazione, lettura del pensiero o sensori psichici. Anche la realtà stessa sembra dimenticarsi di te; i nemici hanno svantaggio ai tiri di Intelligenza (Indagine) per ricordare i dettagli del tuo aspetto. |

#### Archetipo: Operativo
| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Iniezione Letale** (Grado 4) | 1 azione | Tocco | Istantanea | Colpisci con un ago nanotecnologico (attacco in mischia). Il bersaglio subisce **4d10 danni da veleno** e deve superare un TS Costituzione o essere **Paralizzato** permanentemente (fino a cura specifica). |
| **Maschera Perfetta** (Grado 6) | 1 minuto | Tocco | 24 ore | Assumi fisicamente e vocalmente l'identità di un umanoide che hai toccato (anche morto). Ottieni accesso ai suoi scan biometrici (impronte, retina). Indistinguibile dalla realtà. |
| **Protocollo Omega** (Grado 9) | 1 azione | Personale | 1 minuto | Per la durata, ogni colpo che metti a segno è un **Critico**. Non puoi essere sorpreso e hai resistenza a tutti i danni. Al termine, subisci 1 livello di indebolimento. |

#### Archetipo: Artificio
_Nota: L'Artificio usa la lista Melder, ma queste sono tecniche esclusive create tramite i loro gadget unici._

| Nome | Tempo di attivazione | Gittata | Durata | Descrizione |
| :--- | :--- | :--- | :--- | :--- |
| **Sovraccarico Sinaptico** (Grado 4) | 1 azione | Cono (9m) | Istantanea | Il tuo impianto emette un urlo psichico amplificato tecnologicamente. Infligge **6d8 danni psichici** e le creature sono **Stordite** fino alla fine del tuo prossimo turno. |
| **Costrutto Override** (Grado 6) | 1 azione | 18 metri | 1 ora | Prendi il controllo totale di un automa o veicolo entro la gittata (TS Intelligenza nega). Il costrutto agisce nel tuo turno obbedendo ai comandi mentali. |
| **Ancora di Realtà Portatile** (Grado 8) | 1 azione | 18 metri | 1 ora | Piazzi un dispositivo che stabilizza la realtà (raggio 9m). All'interno dell'area, nessun teletrasporto, cambiamento di forma, o viaggio planare è possibile. Le creature eteree diventano tangibili. |
...
# Appendice D: Multiclasse e Talento
## Opzioni di Personalizzazione: Multiclasse & Talenti

### Regole Multiclasse
Il livello totale del personaggio è la somma dei livelli di tutte le classi possedute.
...
### Talenti (Feats)
_Opzione alternativa all'Aumento dei Punteggi di Caratteristica._
...
## Appendice Logica per Antigravity (Backend)

Queste sono le regole logiche da implementare nel codice della scheda per gestire il multiclasse e i talenti.

### 1. Logica Multiclasse

- **Gestione Livello:** `char_level` non è più un input diretto, ma è = `SUM(class_levels)`.
    
- **Aggiunta Classe:** Quando l'utente clicca "Aggiungi Classe":
    
    1. **Check Prerequisiti:** Controlla se `stats_base` del PG soddisfa i requisiti della nuova classe (vedi tabella Prerequisiti). Se `False`, mostra errore/disabilita.
        
    2. **Check Competenze:** Se `total_level > 1`, assegna le competenze dalla tabella _Competenze Multiclasse_, NON quelle della classe base livello 1.
        
- **Slot Incantesimi (Formula Combinata):**
    
    - Crea variabile `caster_level_tech` = `Livelli_Ingegnere` + `floor(Livelli_Cacciatore / 2)` + `floor(Livelli_Sentinella / 2)`.
        
    - Usa la tabella standard degli slot incantesimi (tipo D&D 5e) basata su `caster_level_tech`.
        
- **Punti Talento (Formula Combinata):**
    
    - Crea variabile `total_psi_level` = `Livelli_Melder` + `Livelli_Mistico` + `Livelli_Adepto` + `Livelli_Cybermante` + `floor(Livelli_Specialista_Artificio / 3)` + `floor(Livelli_Guerriero_Paragon / 3)`.
        
    - Usa la tabella Punti Talento del Melder basata su `total_psi_level`.
        

### 2. Logica Talenti (Feats)

- **Attivazione:** Il pulsante "Aggiungi Talento" deve essere disponibile quando una classe raggiunge i livelli di _Aumento Caratteristica_ (solitamente 4, 8, 12, 16, 19, ma vedi Guerriero/Solariano).
    
- **Filtraggio:** Il modale di selezione dei talenti deve nascondere o disabilitare i talenti i cui prerequisiti non sono soddisfatti (es. "Richiede Forza 13" o "Richiede Razza: Vesk").
    
- **Applicazione Effetti:**
    
    - Se il talento dice "+1 a una caratteristica", deve aprire un mini-popup per chiedere _quale_ caratteristica aumentare.
        
    - Se il talento da "Competenza", deve spuntare automaticamente la casella nella lista abilità/TS.
        
    - Se il talento da "Vantaggio", deve aggiungere un tag visivo o un'icona accanto alla statistica relativa.